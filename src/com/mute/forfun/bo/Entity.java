...
public class Entity {
	...
	public void initEntityByTamplate(EntityTamplate tamplate) throws Exception {
		...
		HashMap<UUID, Part> myParts = new HashMap<UUID, Part>();
		...
		for (PartTamplate i : mustParts) {
			...
			myParts.put(part.getPartUUID(), part);
		}
		if (niceParts != null) {
	...
	public boolean isAlive() {
		...
		while (iter.hasNext()) {
			...
			Part val = (Part) entry.getValue();
			if (val.getType() == StaticInputs.PART_TYPE_CONTROL && val.getHP() > 0) {
				aliveFlagCtrl = true;
				break;
			} else if (val.getType() == StaticInputs.PART_TYPE_FATAL && val.getHP() > 0) {
				aliveFlagFatal = true;
				break;
			}
	...
	public int[] actionDecision() throws Exception {
		...
		while (iter.hasNext()) {
			...
			Part val = (Part) entry.getValue();
			if (val.getType() == StaticInputs.PART_TYPE_CONTROL && val.getHP() > 0) {
				controlPoint = controlPoint + val.getFP();
			}
		}
	...
	private int[] getActionListByIQ(int iq) throws Exception {
		...
		case StaticInputs.IQTYPE_NORMAL:
			if (MathUtil.isHitPecentage(70)) {// 70%概率行动，行动中10%概率会校准敌人
				...
				if (MathUtil.isHitPecentage(50)) {
					...
					result[1] = StaticInputs.ACTIONDECISION_SHOTEMENY;
				}
				if (MathUtil.isHitPecentage(50)) {
					result[2] = StaticInputs.ACTIONDECISION_RANDOMMOVE;
				} else {
					result[2] = StaticInputs.ACTIONDECISION_MOVETOEMENY;
				}
		...
		case StaticInputs.IQTYPE_TALENT:
			...
			if (MathUtil.isHitPecentage(50)) {
				result[2] = StaticInputs.ACTIONDECISION_REFRESHEMENYLIST;
			} else {
				result[0] = StaticInputs.ACTIONDECISION_RANDOMSHOT;
			}
			if (MathUtil.isHitPecentage(50)) {
